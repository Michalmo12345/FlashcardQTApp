...
Set::Set(std::string name, std::string creationDate,
         std::string creatorUsername)
    : name_(std::move(name)),
      creationDate_(std::move(creationDate)),
      creatorUsername_(std::move(creatorUsername)) {}

Set::~Set() = default;

std::string Set::getName() const { return name_; }

std::string Set::getCreationDate() const { return creationDate_; }

std::string Set::getCreatorUsername() const { return creatorUsername_; }

void Set::setName(const std::string& name) { name_ = name; }
...
void Set::saveToDB(const std::string& username) const {
  try {
    ...
    pqxx::work txn(*conn);
    std::string insert_set =
        "INSERT INTO set (name, creation_date, creator_id) VALUES ($1, $2, $3)";
    ...
    std::string insertFlashcard =
        "INSERT INTO flashcard (set_id, question, answer, question_file_name, "
        "answer_file_name) VALUES ($1, $2, $3, $4, $5)";
    std::string insertFlashcardBothFiles =
        "INSERT INTO flashcard (set_id, question, answer, question_file, "
        "question_file_name, answer_file, answer_file_name) VALUES ($1, $2, "
        "$3, $4, $5, $6, $7)";
    std::string insertFlashcardQuestionFile =
        "INSERT INTO flashcard (set_id, question, answer, question_file, "
        "question_file_name, answer_file_name) VALUES ($1, $2, $3, $4, $5, $6)";
    std::string insertFlashcardAnswerFile =
        "INSERT INTO flashcard (set_id, question, answer, question_file_name, "
        "answer_file, answer_file_name) VALUES ($1, $2, $3, $4, $5, $6)";
    ...
    for (const auto& card : flashcards_) {
      ...
      if (questionData && answerData) {
        txn.exec_params(insertFlashcardBothFiles, set_id, card->getQuestion(),
                        card->getAnswer(), *questionData,
                        trimFromLastSlash(card->getQuestionFile()), *answerData,
                        trimFromLastSlash(card->getAnswerFile()));
      } else if (questionData && !answerData) {
        txn.exec_params(insertFlashcardQuestionFile, set_id,
                        card->getQuestion(), card->getAnswer(), *questionData,
                        trimFromLastSlash(card->getQuestionFile()),
                        trimFromLastSlash(card->getAnswerFile()));
      } else if (!questionData && answerData) {
        txn.exec_params(insertFlashcardAnswerFile, set_id, card->getQuestion(),
                        card->getAnswer(),
                        trimFromLastSlash(card->getQuestionFile()), *answerData,
                        trimFromLastSlash(card->getAnswerFile()));
      } else {
        txn.exec_params(insertFlashcard, set_id, card->getQuestion(),
                        card->getAnswer(),
                        trimFromLastSlash(card->getQuestionFile()),
                        trimFromLastSlash(card->getAnswerFile()));
      }
    ...
    while (std::getline(file, line)) {
      ...
      if (std::getline(iss, question, ',') && std::getline(iss, answer)) {
        std::shared_ptr<Flashcard> card =
            std::make_shared<Flashcard>(question, answer);
        set->addCard(card);
      }
    }
    file.close();
...
std::unique_ptr<Set> getSetByName(const std::string& setName) {
  try {
    std::string sql =
        "SELECT flashcard.question, flashcard.answer, flashcard.question_file_name, flashcard.answer_file_name, flashcard.id \
    ...
    for (pqxx::result::const_iterator c = R.begin(); c != R.end(); ++c) {
      ...
      if (card->getAnswerFile() != "") {
        ...
        if (!std::filesystem::exists(card->getAnswerFile())) {
          downloadFileFromDatabase(N, card->getAnswerFile(), c[4].as<int>(),
                                   answer_file_sql);
        }
      }
      set->addCard(card);
